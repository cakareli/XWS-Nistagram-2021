...
func handleFunc(handler *handler.FollowHandler) {
	...
	router.HandleFunc("/unmute-following/{loggedUserId}/{followingId}", handler.UnmuteFollowing).Methods("PUT")
	...
	router.HandleFunc("/users-for-feed/{loggedUserId}", handler.FindAllFeedUsers).Methods("POST")
